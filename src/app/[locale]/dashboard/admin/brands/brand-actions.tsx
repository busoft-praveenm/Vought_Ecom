...
import { updateBrandAction, deleteBrandAction } from "@/app/actions/brand";
...
export function BrandStatusToggle({ brand }: { brand: any }) {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();

  const handleToggleActive = async () => {
    setIsLoading(true);
    const res = await updateBrandAction(brand.id, { isActive: !brand.isActive });
    setIsLoading(false);
    if (res.success) {
...
export function BrandActions({ brand }: { brand: any }) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);


  const handleDelete = async () => {
    if (!confirm("Are you sure you want to delete this brand? Products under this brand will be soft deleted.")) return;
    setIsLoading(true);
  ...
  return (
    <div className="flex items-center justify-end space-x-2">
      <Link href={`/dashboard/admin/brands?edit=${brand.id}`}>
